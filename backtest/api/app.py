...
from backtest.config import WEB_HOST, WEB_PORT, PKL_DATA_DIR
...
# 静态文件 & 模板
# 修改为指向 backtest 根目录下的 static 和 templates
...
async def hold_loss_page(request: Request):
    """扛单行为分析页"""
    return templates.TemplateResponse("hold_loss.html", {"request": request})


@app.get("/stoploss", response_class=HTMLResponse)
async def stoploss_page(request: Request):
    """止损回测页"""
    return templates.TemplateResponse("stoploss.html", {"request": request})


@app.get("/position", response_class=HTMLResponse)
async def position_page(request: Request):
    """仓位策略分析页"""
    return templates.TemplateResponse("position.html", {"request": request})
